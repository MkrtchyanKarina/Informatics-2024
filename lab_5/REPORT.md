...
3) Проверим наличие папки .git/hooks/ командой ( -a - для отображения скрытых файлов, -l - для отображения подробной информации о каждом файле: дата последнего изменения и тип файла )
   ```
   ls -la .git/hooks/
   ```
